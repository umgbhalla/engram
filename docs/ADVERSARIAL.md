...
| 2 — Resource bombs (14) | SURVIVED 14/14 | SURVIVED 14/14 | SURVIVED 13/14, **1 BREACH** (array-alloc) |
...
| 6 — Rust-slice resilience (21) | n/a | n/a | SURVIVED 21/22 (= array-alloc breach above) |
...
## Confirmed breaches, ranked by severity
...
### BREACH-2 (MEDIUM/HIGH-on-workerd) — Rust slice incremental fast-array alloc bypasses native memory_limit  [Suite 6]
**Severity: MEDIUM locally / HIGH if shipped to workerd** (same monotonic-buffer DO-kill class).
**Where:** local Rust slice only (`experiments/rustkernel/`).
**Mechanism (cited):** `arr.push(new Array(1e5).fill(7))` drives WASM linear memory to the full **4GB wasm32 ceiling** (65528 pages, used_heap ~3.1GB) **before** rquickjs `set_memory_limit(64MB)` ever trips — the native limit is **effectively bypassed for fast-array storage**. Locally Node grew to 4GB then threw a catchable `InternalError` (process survived, next eval works), **but on workerd the 128MB/1GB isolate cap means an UNCATCHABLE OOM / whole-DO kill long before**. The advertised memory guard does **not** bound this alloc shape. The Rust slice has **no equivalent mid-cell linear-buffer tripwire** — the exact guard the JS kernel added in v0.8 to close this same gap.
...
- **Is Rust structurally safer? At parity on the things that matter, modestly better on two, NOT better on the two real production holes.**
...
- Structurally **NOT safer** on the two production-relevant gaps: (1) **monotonic-linear-memory incremental-alloc DO-kill** — identical exposure, and the Rust slice currently **lacks the mid-cell tripwire** the JS kernel already shipped (BREACH-2); (2) **workerd interrupt-throttle** — the JS kernel's real escape edge was workerd throttling the host interrupt callback after ~1.6k invocations/turn; the Rust slice's interrupt budget was only proven under local Node, **never under workerd**, so its budget-guard advantage is unproven on the real substrate. Borrow checker / type safety buy Rust-side memory safety but **nothing** against guest-JS resource exhaustion (which lives in QuickJS C).
- **Verdict:** a Rust rewrite is at-parity-to-modestly-better on safety, but does **not inherently close** the two production gaps. To match v0.8 it MUST add: (a) a mid-cell `memory.buffer.byteLength` grow tripwire inside the interrupt handler, (b) snapshot manifest/checksum + used-heap admission, (c) the `writeStr` bounds check. Until then Rust's structural advantages do not justify a rewrite on resilience grounds alone.
...
**Bottom line:** the deployed-equivalent kernel (`engram-adv-cur` ≈ `engram-kernel`) survived all 6 suites with **zero breaches**. The single HIGH breach (BREACH-1) lives **only** in the experimental W4 delta variant, which is **not deployed** and which regressed by dropping the size guard. **No breach was found against the live-equivalent kernel.** Recommended hardening before W4 could ship: re-add the 18MB `MAX_DUMP_BUFFER_BYTES` clean-reject; add a mid-cell tripwire + manifest checksum to any Rust kernel; make `{t:evict}` acquire the mutex.
...
- No git commit. No edits to `apps/`. No live worker redeployed.
