...
## ADR-0013 — Raise the engine SCRATCH buffer 1 MB → 32 MB so big payloads cross into the VM (+ the snapshot fences that follow from it)

**Status:** Accepted (live on `engram-kernel-ouru`; prod `engram-kernel` untouched).

**Context.** ADR-0012 made `host.fetch` binary-safe and raised the fetch body cap to 32 MB, but left the engine's **fixed 1 MB SCRATCH** buffer in place. SCRATCH (`scratch_ptr()`/`scratch_cap()` in `engine/src/lib.rs`) is the engine↔shim IO buffer for eval source IN and **host-call results IN on resume** (the fetch body, base64-encoded, is written into SCRATCH on `eval_resume`). So a payload larger than 1 MB could be *fetched* (32 MB cap) but could not *cross into the VM*: the exact failing case — fetch the arXiv "Attention Is All You Need" PDF, `new Uint8Array(await res.arrayBuffer())`, `fs.writeFileSync('/paper.pdf', u)` — died with `ProtocolSizeError: payload …B exceeds scratch capacity 1048576B`.

**Decision.** Raise SCRATCH **1 MB → 32 MB** (`const SCRATCH_CAP = 32 << 20`; both the `static mut SCRATCH` allocation and `scratch_cap()` derive from it). The 32 MB is zero-filled BSS — it does **not** bloat the engine wasm (764 KB). Because SCRATCH lives in the SAME linear memory the W4 snapshot blits, raising it forces three follow-on fences (all in `kernel-glue.ts`), each measured against real `workerd`:
1. **Per-cell instruction budget 1200 → 500000** (`cellBudgetTicks` default). Base64-decoding a multi-MB body burns far more bytecode than a normal cell; the old 1200 tripped a `TimeoutError` (~150k ticks observed for the 2.2 MB PDF) before the cell could finish. Overridable per session.
2. **Per-cell linear-memory grow cap 128 pages/8 MB → 1024 pages/64 MB** (`cellGrowCapPages` default). A cell may legitimately pull in a fetch body up to the 32 MB fetch cap plus working copies; the old 8 MB cap tripped a `MemoryLimitError`. The rquickjs heap limit (64 MB) + the snapshot size-admission guard remain the hard OOM fences.
3. **Capped utf8 `body` preview + larger snapshot raw-image caps.** `_doFetch` was decoding the FULL body to a utf8 `body` string ALONGSIDE the base64 `bodyB64` in the same host-call result — ~2x the redundant string in the VM (QuickJS strings are UTF-16). Now the utf8 `body` is a **64 KB preview** (`FETCH_BODY_UTF8_PREVIEW_BYTES`, flagged `bodyTruncated`); `.text()`/`.json()` fall back to decoding the exact bytes from `bodyB64` when truncated, so correctness is unchanged. The raw-image (buffer) admission caps (`SAFE_SERIALIZE_BUFFER_BYTES`, `MAX_RESTORE_RAW_BYTES`) were raised by the +31 MB resident-scratch delta to **76 MB** (the USED-HEAP caps are unchanged — they measure the rquickjs allocator tally, which does **not** include the static scratch — so the heap cap is untouched). Finally, because the 32 MB scratch lifts **every** session's buffer to ~55–59 MB, the W4 dump's 3-copy retention (`prev` + `raw` + a fresh retained `_lastImage`) OOMs the DO once a big payload pushes the buffer to ~67 MB; so for a buffer above `LARGE_BUFFER_NO_RETAIN_BYTES` (**63 MB**, set ABOVE the measured ~59 MB normal-session ceiling) `dumpW4` takes a **2-copy no-retain full path** (peak ~raw + gz). A first cut tripped this at 40 MB and reset the engine-migration oplog on *every* normal cell (regression caught by `hostcall-local`); 63 MB leaves ordinary cells on the delta path with their oplog tail intact.

**Why.** A 32 MB scratch is the structural unblocker for any >1 MB payload crossing into the VM (PDFs, large files, tarballs). The follow-on fences are the minimum needed for the **default** `{fetch:true}` config to handle a real multi-MB fetch end-to-end without weakening the OOM/timeout guards for ordinary cells (all overridable per session). The no-retain dump path is the memory-safety fence the resident scratch makes necessary.

**Consequence.** Proven live on `engram-kernel-ouru` (prod `engram-kernel` untouched), engine hash `rust-770c92475148f5e5c47eacfc97d53d7b`: the exact failing case now returns `{bytes:2215244, magic:"%PDF", saved:2215244}` (the arXiv PDF is 2.2 MB per `content-length`; the bug report's "~6.6 MB" was an estimate) — `ok:true`, **no `ProtocolSizeError`**, **`checkpoint.ok:true`**, stable across repeated runs. Binary fetch stays byte-exact (favicon = 6518 B); `.text()` on a small body is unchanged (full, non-truncated). Existing suites stay green: **hostcall-local 12/12, completion-local 11/11**. Trade-off: a single cell that pulls in a 32 MB-class payload trips the no-retain full path and resets the oplog tail, so engine-migration *replay* of that one cell would no-op (the byte-blit image stays the primary restore path) — an accepted edge, not the steady state. A genuinely huge heap session (≈50 MB used heap + 32 MB scratch > the 76 MB raw cap) is refused at snapshot with a recoverable `SizeAdmissionError`, socket alive.
