...
// ---- W4 FINE-GRAIN BYTE-DELTA (docs/W4-BYTEDELTA-PLAN.md) ----
// On checkpoint, diff the current raw QuickJS snapshot image against the previous one
// (`this._lastImage`, retained host-side in the glue instance — NOT in the VM heap, dropped on
// evict, re-derived on cold wake). Emit only the changed 256B grains when the delta is small;
// else emit a full (W5-compacted) base. A full base every BASE_EVERY cells caps the restore chain.
const DELTA_GRAIN_BYTES = 256;   // W4-proven sweet spot (295KB gz delta vs ~1MB full)
const DELTA_FALLBACK_PCT = 0.5;  // delta >= 50% of full => store full instead (auto-fires on dense mutation)
const BASE_EVERY_CELLS = 20;     // full compacted base cadence; bounds the delta-chain length on restore
...
export class GlueKernel {
  constructor() {
    ...
    // W4: the previous raw serialized image, retained host-side (NOT in the VM heap) so the next
  ...
  // W5: GC + used-heap admission + freed-arena scrub, then snapshot+serialize. Shared by the W5
  // full-image dump() and the W4 byte-delta dumpW4(). Returns the RAW serialized image + meta.
  // docs/W5-COMPACTION-PLAN.md + docs/W4-BYTEDELTA-PLAN.md.
  _serializeForDump(QuickJS) {
    // W5 UN-WEDGE: HARD safe-to-serialize ceiling FIRST — before runGC/any full-buffer touch
    // (snapshot+serialize is ~2-3x; uncatchable OOM near the real ~48MB cliff).
    ...
    // W5 (a): admission on the ACTUAL used heap, NOT the monotonic buffer.
    ...
    // W5 (b)+(c): scrub freed pages to zero so the monotonic buffer gzips down.
    let scrubbed = false;
    ...
    return { serialized, usedHeap: usedAfterGc, bufferBytes: bufBytes, scrubbed, snap };
  }

  // Dump memory+globals -> serialize -> STREAM gzip. (W5 full-image path; kept for the W5-parity
  // wedgeTest hook + as the W4 base.) P0 (BUG-2/4): GC, guard on USED heap, then SCRUB the freed
  // arena to shrink the gz image. docs/glue-changelog.md.
  async dump() {
    const QuickJS = await QJS();
    const { serialized, usedHeap, bufferBytes, scrubbed, snap } = this._serializeForDump(QuickJS);
    // W4: retain the raw image host-side so the NEXT checkpoint can byte-diff against it.
    this._lastImage = serialized.slice();
  ...
  // W4 BYTE-DELTA dump. Diffs the current raw image against the retained `this._lastImage` at a
  // 256B grain. If a prior image exists AND `forceFull` is false AND the delta is small
  // (< DELTA_FALLBACK_PCT of full) => emit `mode:"delta"` carrying ONLY the changed grains + their
  // indices. Else => emit `mode:"full"` (a W5-compacted base) and reset the chain. The host
  // (lib.rs) decides forceFull (cell % BASE_EVERY == 0 OR no committed base). On a length change
  // (image grew/shrank) we always emit a full base — the grain indexing assumes equal length.
  // docs/W4-BYTEDELTA-PLAN.md.
  async dumpW4(forceFull) {
    ...
    const canDelta =
      !forceFull && prev && prev.byteLength === serialized.byteLength;

    if (canDelta) {
      // Content-chunked diff: collect indices of 256B grains that differ.
      const grain = DELTA_GRAIN_BYTES;
      const nGrains = Math.ceil(serialized.byteLength / grain);
      const changed = [];
      for (let i = 0; i < nGrains; i++) {
        const start = i * grain;
        const end = Math.min(start + grain, serialized.byteLength);
        let diff = false;
        for (let j = start; j < end; j++) {
          if (serialized[j] !== prev[j]) { diff = true; break; }
        }
        if (diff) changed.push(i);
      }
      // Pack changed grains contiguously + a Uint32 index list.
      const payload = new Uint8Array(changed.length * grain);
      for (let k = 0; k < changed.length; k++) {
        const i = changed[k];
        const start = i * grain;
        const end = Math.min(start + grain, serialized.byteLength);
        payload.set(serialized.subarray(start, end), k * grain);
      }
      ...
      // AUTO-FALLBACK: a dense mutation (delta >= FALLBACK_PCT of a full image) is not worth it —
      // store a full base instead. We approximate "full" cost by the gz of the whole image.
      if (deltaStored < serialized.byteLength * DELTA_FALLBACK_PCT) {
        this._lastImage = serialized.slice();
        return {
          mode: "delta",
          gz: payGz,
          ...
          grain,
          imageLen: serialized.byteLength,
          sizeGz: deltaStored,
          ...common,
        };
      }
      // fall through to full base
    }
  ...
  // W4 restore: reconstruct the raw image from a gz'd full base + an ordered chain of gz'd deltas,
  // then deserialize+blit (same path as restore()). `deltaList` = [{gz, indicesGz, grain}] applied
  // in order. The reconstructed image is retained as `this._lastImage` so the next warm checkpoint
  // diffs correctly. Engine-hash / used-heap / raw-ceiling admission identical to restore().
  // docs/W4-BYTEDELTA-PLAN.md.
  async restoreW4(baseGz, deltaListJson, engineHashOfSnap, clockCalls, rngCalls, configJson, label, kvJson, usedHeap, ctxJson) {
    const tEnter = Date.now();
    const CUR = engineHashValue();
    if (engineHashOfSnap && engineHashOfSnap !== CUR) {
      throw new EngineHashMismatchError(engineHashOfSnap, CUR);
    ...
    const QuickJS = await QJS();
    // Reconstruct: base, then apply deltas in order.
    const base = await gunzip(baseGz);
    let image = base; // Uint8Array, mutated in place by deltas
    const deltas = Array.isArray(deltaListJson) ? deltaListJson : [];
    for (const d of deltas) {
      const payload = await gunzip(d.gz);
      const idxBytes = await gunzip(d.indicesGz);
      const grain = d.grain || DELTA_GRAIN_BYTES;
      // idxBytes is a packed Uint32Array; align-copy into a fresh Uint32Array (offset may be odd).
      ...
      for (let k = 0; k < nIdx; k++) idx[k] = dv.getUint32(k * 4, true);
      for (let k = 0; k < nIdx; k++) {
        const i = idx[k];
        const start = i * grain;
        const end = Math.min(start + grain, image.byteLength);
        image.set(payload.subarray(k * grain, k * grain + (end - start)), start);
      }
    }
    ...
    let cfg = {};
    if (configJson) { try { cfg = JSON.parse(configJson); } catch (_) {} }
    this._applyConfig(cfg);
    this._hydrateKv(kvJson);
    this._hydrateContext(ctxJson);
    const snap = QuickJS.deserializeSnapshot(serialized);
    ...
    reRegisterHostFns(this.kernel, this.ctx);
    this.kernel.executePendingJobs();
    // W4: retain the reconstructed raw image so the next warm checkpoint diffs against it.
    this._lastImage = serialized.slice();
    this._restoreTimings = {
      gunzipMs: tAfterGunzip - tEnter,
      instantiateMs: tAfterInstantiate - tBeforeInstantiate,
      ...
      growCount,
      growDeltaPages,
      deltaChainLen: deltas.length,
    };
    return label || "w4-delta-restore";
  }
