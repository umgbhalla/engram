...
const MAX_RESTORE_RAW_BYTES = 18 * 1024 * 1024; // safe-to-instantiate raw ceiling (lockstep w/ dump)
const SCRUB_SLACK_BYTES = 4 * 1024 * 1024; // scrub freed arena when buffer-usedHeap exceeds this
const SCRUB_MAX_BUFFER_BYTES = 16 * 1024 * 1024; // only scrub below this (must stay < dump ceiling)
const MAX_DUMP_BUFFER_BYTES = 18 * 1024 * 1024; // hard dump ceiling on the monotonic linear buffer
...
export class GlueKernel {
  ...
  async dump() {
    ...
    // P0: HARD buffer ceiling FIRST — before runGC/any full-buffer touch (snapshot+serialize is
    // ~2-3x; uncatchable OOM). Fail safe (typed error, socket alive) past it. docs/glue-changelog.md.
    ...
    // P0: admission on the ACTUAL used heap (un-wedges checkpointing after a free).
    ...
    // P0: shrink the stored image after a free — scrub freed pages to zero (only below the
    // ceiling so the bounded, non-growing scrub can't OOM). docs/glue-changelog.md.
    let scrubbed = false;
    if (
