...
class GlueKernel {
  ...
  async restoreW4(baseGz, deltaList, engineHash, clockCalls, rngCalls, configJson, label, kvJson, usedHeap, ctxJson) {
    ...
    let image = base; // mutated in place by deltas
  ...
  async dumpW4(forceFull) {
    ...
    const canDelta = !forceFull && prev && prev.byteLength === raw.byteLength;
    ...
    if (canDelta) {
      ...
      for (let i = 0; i < nGrains; i++) {
        ...
        let diff = false;
        for (let j = start; j < end; j++) { if (raw[j] !== prev[j]) { diff = true; break; } }
