...
function hostFnDefs(ctx) {
  return {
    ...
    __hostCall(...args) {
      ...
      return this.vm.newString(JSON.stringify(result));
    },
  };
}
...
function buildToolRegistry(ctx, toolNames) {
  ...
  const enabled = {};
  const names = Array.isArray(toolNames) && toolNames.length ? toolNames : Object.keys(all);
...
export class GlueKernel {
  ...
  _applyConfig(cfg) {
    ...
    this.ctx.kv = this.ctx.kv || new Map();
    // P3: expose the live config (fetch allowlist) to host callbacks.
  ...
  async restore(gzU8, engineHashOfSnap, clockCalls, rngCalls, configJson, label, kvJson, usedHeap) {
