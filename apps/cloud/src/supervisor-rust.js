...
export class SupervisorDO extends DurableObject {
  ...
  async _resolveLocal(rawKey) {
    ...
    const row = this.ctx.storage.sql
      .exec("SELECT tenant_id, plan, revoked FROM tenants WHERE api_key_hash=?;", hash).toArray()[0];
    if (!row || row.revoked) return null;
    return { tenantId: row.tenant_id, plan: row.plan, hash };
  }
  async authResolve(rawKey) { return await this._resolveLocal(rawKey); }
  async _revokeKey({ apiKey, apiKeyHash, tenantId }) {
    if (apiKey) apiKeyHash = await sha256Hex(apiKey);
    if (apiKeyHash) {
      const r = this.ctx.storage.sql.exec("UPDATE tenants SET revoked=1 WHERE api_key_hash=?;", apiKeyHash);
  ...
  async alarm() {
    const now = Date.now();
    const rows = this.ctx.storage.sql.exec("SELECT * FROM sessions WHERE last_activity > 0;").toArray();
    for (const row of rows) {
      ...
      if (idle >= FACET_IDLE_TTL_MS && !insideHorizon) {
        try { this.ctx.facets.abort(row.facet, new Error("idle TTL eviction (supervisor alarm)")); } catch (_) {}
        this.ctx.storage.sql.exec(
    ...
    try {
      const perTenant = this.ctx.storage.sql
        .exec("SELECT tenant, COUNT(*) AS c FROM sessions WHERE last_activity > 0 GROUP BY tenant;").toArray();
      for (const t of perTenant) this._meter(t.tenant, "facet", t.c | 0);
    } catch (_) {}
    const active = this.ctx.storage.sql
      .exec("SELECT COUNT(*) AS c FROM sessions WHERE last_activity > 0;").toArray()[0].c | 0;
  ...
  async _frame(tenant, sessionId, msg) {
  ...
  async _evalMetered(tenant, sessionId, src, cell) {
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (p === "/warm") {
        ...
        const row = this.ctx.storage.sql
          .exec("SELECT * FROM sessions WHERE facet=?;", facetName(tenant, sessionId)).toArray()[0] || null;
...
function authStub(env) { return env.SUPERVISOR.get(env.SUPERVISOR.idFromName(AUTH_SHARD)); }
  ...
  async fetch(req, env) {
    ...
    if (p === "/usage") {
      ...
      if (!isAdmin) {
        ...
        const resolved = await authStub(env).fetch(`https://do/_auth/resolve?key=${encodeURIComponent(key)}`).then((r) => r.json()).catch(() => null);
    ...
    const resolved = await authStub(env)
      .fetch(`https://do/_auth/resolve?key=${encodeURIComponent(key)}`).then((r) => r.json()).catch(() => null);
