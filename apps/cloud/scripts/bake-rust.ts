...
const indexJs: string = readFileSync(join(kernelRust, "build", "index.js"), "utf8");
...
const engineHash: string = engineHashMatch ? engineHashMatch[1]! : "unknown";

// stdlib bundle + meta (the Rust kernel ships these too; the cloud reuses the cloud copies).
const stdlibBundleTxt: string = readFileSync(join(src, "stdlib.bundle.txt"), "utf8");
let stdlibMeta: string;
try {
...
const out = `// GENERATED by scripts/bake-rust.ts — do not edit. TRACK B (Rust kernel as facet).
