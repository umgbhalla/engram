...
**Deployed CF resources (current):** `engram-kernel` (codemode kernel + **compute-edge substrate**: muscle-ladder, unified `/workspace` fs, Worker-Loader `LOADER` binding for the Tier-1 hash-worker registry, `ENGRAM_SANDBOX_URL`/`KEY` for the Tier-2 binding; `apps/kernel/wrangler.jsonc` + `alchemy.run.ts` — **deploy via `alchemy deploy`, NOT wrangler** [DO-migration 10074]) + **`engram-sandbox`** (NEW Tier-2: `@cloudflare/sandbox` Linux container, `apps/sandbox/`, `wrangler deploy`) + `engram-cloud` (**multi-tenant** parity: facets + stdlib/extensions/mediated-fetch/adaptive-keep-warm; `apps/cloud/wrangler.jsonc`) + `engram-ui` (notebook SPA). All older/spike/experiment workers swept. R2 `engram-snapshots` (stale keys remain; needs R2 S3 token to prune). AE dataset `engram_kernel`. Workers Paid. Repo: private `github.com/umgbhalla/montydyn` (on-disk folder/repo kept `montydyn` per the rebrand note above ⚠ verify against your git remote). Monorepo layout: bun workspaces `apps/*` + `packages/*` (see Code map above) — older `v0.x/` trees pruned; in git history + `v0.N-milestone` tags.
...
**COMPUTE-EDGE SUBSTRATE (2026-06, this arc — `docs/SESSION-SUMMARY.md` + `docs/research/{COMPUTE-EDGE-AND-SUBSTRATE,UNIFIED-FS,RLM-EXEC-ENVIRONMENTS,ACTION-ITEMS,ROADMAP-TODO}.md`):** engram graduated from "durable REPL" to a **capability-brokered compute substrate** — a durable QuickJS *brain* that brokers an escalating **muscle ladder** over one R2 data plane, every rung capability-injected fail-closed.
- **Tier-1 content-addressed Worker-Loader registry** (live): `s.registerWorker(src)→sha256`, `s.invokeWorker(hash,input)` runs in a fresh isolate with a prefix-scoped `env.VFS` (`VfsGateway` WorkerEntrypoint, `globalOutbound:null`). `LOADER` worker-loaders binding on `engram-kernel`.
- **Tier-2 sandbox** (live, NEW worker `engram-sandbox`, `apps/sandbox/`): `@cloudflare/sandbox` real Linux container; `s.sandbox.exec/git/expose` from a cell, capability-gated, sharing the SAME R2 `/workspace`. Kernel reaches OUT via a DO-side fetch (`ENGRAM_SANDBOX_URL`+`ENGRAM_SANDBOX_KEY` secret) — additive, no kernel rewrite.
- **Unified `/workspace` filesystem** (live): ONE root everywhere — a cell's `fs.writeFile('/workspace/x')`, a hash-worker's `env.VFS`, and the container's `/workspace/x` are the SAME R2 object (`fs/<doId>/x`). CWD semantics (`process.cwd/chdir` clamped under `/workspace`, relative resolves against cwd via `__wsResolveAbs`); backward-compatible. **`packages/engram-fs`** = the extraction-ready canonical resolver/manifest/coherence core (own repo later).
- **Capabilities/fixes (live):** net/tls raw sockets (cloudflare:sockets) · VFS host↔R2 file I/O · **TS via sucrase** (generics/enums) · sha512/384 · nanoid ESM · clock:real (real inter-cell) · stack-guard (8MB wasm stack + 256KB JS guard → deep recursion catchable, no instance corruption).
- **RLM substrate:** the surveyed RLM exec-envs (Zhang in-process/IPython/Docker, Daytona microVM) are all EPHEMERAL (`dill` is their durability ceiling; nobody forks). engram's edge = ONE mechanism (WASM linear memory) is isolation+state+checkpoint, so a recursion tree hibernates+resumes. Canonical RLM primitive = `host.spawn`+`eval(slice)`; **`host.fork`** (designed, NOT built) would be novel.
- **R2 bucket note:** the alchemy-deployed kernel + the sandbox both bind `engram-kernel-engram-snapshots-umang` (alchemy-managed name from `adopt:true`); `wrangler.jsonc`'s `engram-snapshots` is latent (a raw `wrangler deploy` mis-binds, but it fails via DO-migration 10074 anyway — deploy via `alchemy deploy`).
- **Deploy lesson:** kernel deploys are done IN-THREAD (cargo build / alchemy deploy exceed the workflow 180s no-progress watchdog). The whole tree is now committed (a stray `git reset` cost uncommitted engine source once — recovered).
- **Open (NOT built):** `host.spawn`/`host.fork` (staged) · #4 sync-over-async VFS · net/tls keepalive read-block (host-side) · Tier-3 Workflows / R2-event-Queue wake-loop. Known limits: native-builtin alloc bombs bypass the interrupt tripwire; `host.kv`/`s.set-get` removed in v2; deep-recursion depth bounded (bigger wasm stack breaks the dump ceiling).
