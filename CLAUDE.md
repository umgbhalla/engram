...
- **v0.7 = guards + test-all (NOW):** apply the 3 stdlib guards (esp lower dump ceiling so the ~24–30 MB OOM cliff clean-rejects), then run the full test-all (11-item stdlib checklist + regression + adversarial). Then Tier-0 extensions wire-up (cheap env-fidelity jump).
