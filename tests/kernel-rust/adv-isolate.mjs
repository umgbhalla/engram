...
const RUN = Date.now();
function connect(sid){return new Promise((res,rej)=>{const ws=new WebSocket(`wss://${BASE}/?id=${sid}`);const st={ws,alive:true,code:null};ws.on("open",()=>res(st));ws.on("error",rej);ws.on("close",c=>{st.alive=false;st.code=c});});}
